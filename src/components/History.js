    ...
    <div className='item-details-box-overlay'>
      <div className='item-details-box p-4 col-11 col-sm-5 bg-white mt-5 rounded'>
        ...
        <div className='details mt-3 fs-6 fw-bold'>
          ...
          <div>Date: <ItemDetailsText text={date.month + "/" + date.day + "/" + date.year} /></div>
          <div>ID: <ItemDetailsText text={id} /></div>
        </div>

        <div className='mt-3 '>
          <button className='btn btn-danger' onClick={() => { deleteItem(item,"delate"); onClose(); }}>Delete Item</button>
          <button className='btn btn-primary m-1' onClick={() => { deleteItem(item,"Edit"); onClose(); }}>Edit</button>
        </div>
      </div>
    </div>
  );
